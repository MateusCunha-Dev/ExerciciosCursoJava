package estruturas_sequenciais;

import java.util.Scanner;

public class exercicio06 {
    public static final double pi = 3.14159;
    public static void main(String[] args) {
        /*
        Fazer um programa que leia três valores com ponto flutuante de dupla
        precisão: A, B e C. Em seguida, calcule e mostre:
        a) a área do triângulo retângulo que tem A por base e C por altura.
        b) a área do círculo de raio C. (pi = 3.14159)
        c) a área do trapézio que tem A e B por bases e C por altura.
        d) a área do quadrado que tem lado B.
        e) a área do retângulo que tem lados A e B.
        */

        Scanner sc = new Scanner(System.in);
        double a, b, c;

        System.out.print("Digite o valor de A: ");
        a = sc.nextDouble();
        System.out.print("Digite o valor de B: ");
        b = sc.nextDouble();
        System.out.print("Digite o valor de C: ");
        c = sc.nextDouble();

        double areaRightTriangle = (a * c)/2;
        double areaCircle = pi * Math.pow(c, 2);
        double areaTrapezoid = (a + b) * c/2;
        double areaSquare = Math.pow(b, 2);
        double areaRectangle = a * b;

        System.out.print(
                "\nArea do triângulo retângulo = " + areaRightTriangle + "\n" +
                "Area do círculo = " + areaCircle + "\n" +
                "Area do trapézio = " + areaTrapezoid + "\n" +
                "Area do quadrado = " + areaSquare + "\n"+
                "Area do retângulo = " + areaRectangle
        );
        sc.close();
    }
}
